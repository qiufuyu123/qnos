...
int init_vfs()
{
    if(!fs_ops_list[0]||!root_sb)return -1;
...
{
    vfs_inode_t *inode=inode2ptr(file);
    if(inode->magic_num!=INODE_MAGIC_NUM)return 0;
    return inode->sb;   
}
...
char *__travel_path(char *path,int *cnt,int len)
{
    ...
    if(*cnt>=len)return __CHAR_NONE;
    while (1)
    {
        if(*cnt>=len)return old_path;
        if(*path=='/')
        {
...
int vfs_fread(vfs_file_t *file,uint32_t size,uint32_t buffer)
{
    if(file->open_flag&O_RDONLY||file->open_flag&O_RDWR)
    {
        ...
        return inode->i_ops->fs_read(&inode->inode_ptr,size,buffer,0);   
    }
...
vfs_file_t *vfs_fopen(char *path,uint8_t flag)
{
    ...
    return d_elem->d_file;
}
int sys_read(int fd,char *buffer,uint32_t size)
{
    if(fd<0)return VFS_BAD_ARG_ERR;
    IN_LOCK
...
{
    if(!path)return VFS_BAD_ARG_ERR;
    IN_LOCK
...
int init_fslist()
{
    ...
    vfs_file_t*file= vfs_fopen("/boot/grub/./../grub/grub.cfg",O_RDONLY);
    if(file)
    {
        printf("[VFS] open cfg ok!;");
        char *text=kmalloc(30);
        int s= file->ops->read(file,30,text);
        printf("[%s][%d]",text,s);
    }
