...
void put_textbox(char *str,int x,int y,int w,int h)
{
    int cx=x;
    int cy=y;
    fill(x,y,w,h,0xF5F5DC);
    vbg=0xF5F5DC;
    vfg=0;
    while (*str)
    {
...
int main()
{
    ...
    int fd3=open("/boot/sys/test.txt",O_RDONLY);
    ...
    uint32_t based=cfg.width*cfg.height*4-20*8*cfg.width*4; //最底下的3行字符扣掉
    uint32_t v2= based&0xfffff000;
    uint32_t offset=based-v2;
    vb_pgcnt=ngx_align(15*8*cfg.width*4,4096)/4096;
    width=cfg.width;
    height=cfg.height;
    ...
    {
        sleep(1000);
        put_textbox(sbuf,500,0,8*20,8*14);
    }
